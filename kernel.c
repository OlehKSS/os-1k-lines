...
struct process *create_process(const void *image, size_t image_size) {
    ...
    *--sp = (uint32_t) user_entry;  // ra
    ...
    // Map MMIO region for access to MMIO registers.
    man_page(page_table, VIRTIO_BLK_PADDR, VIRTIO_BLK_PADDR, PAGE_R | PAGE_W);
...
void handle_syscall(struct trap_frame *f) {
    ...
    case SYS_EXIT:
        // WARNING:
        // For simplicity, we are directly referencing pointers passed from applications (aka. user pointers),
        // but this poses security issues. If users can specify arbitrary memory areas, 
        // they could read and write kernel memory areas through system calls.
...
struct virtio_virtq *virtq_init(unsigned index) {
    ...
    // 1. Select the queue writing its index (first queue 0) to QueueSel.
    virtio_reg_write32(VIRTIO_REG_QUEUE_SEL, index);
    // 5. Notify the device about the queue size by writing the size to QueueNum
    virtio_reg_write32(VIRTIO_REG_QUEUE_NUM, VIRTQ_ENTRY_NUM);
    // 6. Notify the device about the used alignment by writing its value in bytes ot QueueAlign.
 ...
 * @param desc_index is the index of the head descriptor of the new request
 */
...
/**
 * Return true if there are requests being processed by the device.align_up.
 */
...
void read_write_disk(void *buf, unsigned sector, int is_write) {
    ...
    if (blk_req->status != 0) {
        printf("virtio: warn: failed to read/write secor=%d status=%d\n",
...
void fs_init(void) {
    ...
    for (int i = 0; i < FILES_MAX; i++) {
        ...
        // The numbers in the tar header are in octal format. 
